...
behind a C ABI for other languages. All three run the same 58 cores from
...
[`examples/ibea_mixed.cpp`](../examples/ibea_mixed.cpp)); 45 of the 58 cores
accept one, the other 13 refuse it at `setup()` with a message.
...
`#include <mootation/embed.hpp>` instantiates all 58 cores in that translation
...
| `crossover`, `mutation` | words: `sbx` / `uniform` / `blx_alpha`, and `polynomial` / `gaussian` / `cauchy` / `uniform_reset` / `mixture` / `mixture_cauchy` — NSGA-II, IBEA-ε+, SPEA2+SDE and AGE-MOEA take both, MOEA/D-DE the mutation; the defaults are the papers' SBX and polynomial mutation |
